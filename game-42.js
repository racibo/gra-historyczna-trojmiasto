...
function missionLabel(p,category){
  ...
  if(category==="functions")return n.length>100?n.slice(0,97)+"…":n;
  if(category==="names")return n.length>100?n.slice(0,97)+"…":n;
  if(category==="history")return n.length>100?n.slice(0,97)+"…":n;
  if(category==="institutions")return n.length>100?n.slice(0,97)+"…":n;
  if(category==="creators")return n.length>100?n.slice(0,97)+"…":n;
...
function choose(p){choiceEl.classList.add("hidden");candidateMarkers.forEach(m=>map.removeLayer(m));candidateMarkers=[];clearSearchZone();statusEl.style.cursor="";statusEl.title="";current=p;visited.add(p.id);moves++;routePoints.push(p);updateVisitedLabels();updateMoveInfo(moves===1?2:3);updateRoute();setCurrent(p);reveal(p)}
function finish(){
...
function missionHeat(t){
  ...
  const targets=points.filter(p=>p.id!==current.id&&!visited.has(p.id)&&t.test(p));
  if(!targets.length)return ' <span class="heat heat-snow heat-large" title="Brak dostępnego punktu">❄️</span>';
  const nearest=Math.min(...targets.map(p=>distance(current,p)));
  if(nearest<200)return ' <span class="heat heat-fire heat-large" title="poniżej 200 m">🔥</span>';
  if(nearest<700)return ' <span class="heat heat-fire heat-medium" title="200–700 m">🔥</span>';
  if(nearest<1000)return ' <span class="heat heat-fire heat-small" title="700–1000 m">🔥</span>';
  if(nearest<2000)return ' <span class="heat heat-snow heat-small" title="1–2 km">❄️</span>';
  if(nearest<3000)return ' <span class="heat heat-snow heat-medium" title="2–3 km">❄️</span>';
  return ' <span class="heat heat-snow heat-large" title="powyżej 3 km">❄️</span>';
}
function updateMoveInfo(stage){
  if(stage===0)movesEl.textContent="Teraz wybierz za pomocą strzałek ogólny kierunek poruszania się.";
  else if(stage===1)movesEl.textContent="Teraz wybierz jeden z dwóch wariantów pierwszego etapu Twojej podróży.";
  else if(stage===2)movesEl.textContent="Teraz wybierz za pomocą strzałek kierunek kolejnego etapu podróży.";
  else movesEl.textContent="Ruchy: "+moves;
}
