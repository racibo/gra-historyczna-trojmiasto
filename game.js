...
function chooseStartAndTarget(){
  ...
  for(let attempt=0;attempt<80;attempt++){
    ...
    for(const path of candidates){
      const target=path[path.length-1],candidate={start,target};
      if(distanceRangeMatch(candidate))matching.push({start,target,auditMoves:path.length-1,auditedStates:audit.examined});
    }
    if(matching.length>=8)break;
  }
...
function saveSettings(){const oldRange=settings.distanceRange;settings.count=+document.getElementById("missionCount").value;settings.age=document.getElementById("catAge").checked;settings.periods=document.getElementById("catPeriods").checked;settings.architects=document.getElementById("catArchitects").checked;settings.distanceRange=document.getElementById("distanceRange").value;localStorage.setItem("trojmiastoGameSettings",JSON.stringify(settings));return oldRange!==settings.distanceRange}
